...
function* authSaga({ type, payload }) {
  switch (type) {
    case SIGNIN:
      try {
        yield initRequest();
        yield call(firebase.signIn, payload.email, payload.creds);
      } catch (e) {
        yield handleError(e);
      }
      break;
    ...
    case SIGNUP:
      try {
        ...
        const ref = yield call(firebase.createAccount, payload.email, payload.creds);
        const fullname = payload.cname.split(' ').map((name) => name[0].toUpperCase().concat(name.substring(1))).join(' ');
        const user = {
          cname : fullname,
    ...
    case ON_AUTHSTATE_SUCCESS: {
      ...
      if (snapshot.data()) { // if user exists in database
        ...
        console.log('payload at ON_AUTHSTATE_SUCCESS' + JSON.stringify(payload));
        const user = {
          cname: payload.cname, //payload.displayName ? payload.displayName : 'User',
          avatar:  defaultAvatar,
          creds: payload.creds,
          //banner: defaultBanner,
          email: payload.email,
